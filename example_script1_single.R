...
ffeat=t(all_features) #transposes matrix
#colnames(ffeat) <- all_labels #if many files

#cut the name of the inputfile out so you can name the outputfile like that:

write.table(ffeat, file="testfile.csv", row.names = F)
#write.table(ffeat, file=paste("features_", songname, ".csv", sep=""), row.names = F)

#outdir="outdir_name" #this is the directory where we save the output files
#if(!dir.exists(outdir)){dir.create(outdir)}
#write.table(ffeat, file=paste(outdir,"/features_", songname, ".csv", sep=""), row.names = F)
